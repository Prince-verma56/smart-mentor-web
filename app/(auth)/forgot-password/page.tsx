    ...
    <div className="w-full rounded-xl border border-border bg-card p-8 shadow-md">
        ...
        <form onSubmit={handleVerifyAndReset} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <button type="button" onClick={() => setShowPassword(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-slate-600 dark:hover:text-slate-300" aria-label={showPassword ? "Hide" : "Show"}>
          ...
          <button type="button" onClick={() => { setStep("request"); setError(""); }} className="block w-full text-center text-sm text-foreground0 hover:text-violet-600">← Use a different email</button>
        </form>
      )}
      ...
      {step === "done" && (
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          <p className="text-sm text-slate-600 dark:text-muted-foreground">Password changed successfully!</p>
        </div>
      )}
